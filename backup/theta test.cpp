...
int main(int argc, char *argv[]){
    ...
    for(int j = 0;j<3;j++)
    {
        ...
        if(j == 1)gundata[j].setup(false,4,3,2,3,5,6,28,18,3); //pisto
        if(j == 2)gundata[j].setup(true,2,1,2,2,4,10,35,13,5); //submachine
        for(int i = 0; i < 5;i++)
        {
    ...
    while(!key[KEY_ESC])
    {
        ...
        for(int a = 0; a < SCREEN_X; a++){
            ...
            if(light_decrement < 255 && toggle_l == true){
               if(vertical_d < horizontal_d)
                ...
                draw_lit_sprite(wall_temp, Brick, 0, 0, light_decrement);
            ...
            //Floor Casting -----------------------------------------------------------
            int i;
            constant = fFloorTable[floor_angle];
            ...
            if(Middle > SCREEN_Y/2)
            {
                ...
                for(int b = 0; b < camera_y; b++)
                {
                    ...
                    pixel = getpixel(Ground,floor_texture[0], floor_texture[1]);
                    if(toggle_l == true)
                    {
    	...
    	for (int x = 0;x<Display_Sprite.size();x++) {
            if(Display_Sprite[x].distance > 20)
            {
                ...
                {
            		// size of the sprite
            		sprite_size = screen_distance / (cos(sprite_angle*PI/180)*Display_Sprite[x].distance);
                ...
                else if(Display_Sprite[x].distance > 40 || Display_Sprite[x].death_animation || !Display_Sprite[x].isDead())//drawing enemies
                {
            		//size of the sprite
            		sprite_size = screen_distance / (cos(sprite_angle*PI/180)*Display_Sprite[x].distance);
        ...
        Shot = false;
        masked_stretch_blit(weapon[current_gun][player.state], buffer,0,0, 64,64, SCREEN_X/2-200/2, SCREEN_Y-200, 200, 200);
...
void collide_detection(int xpos, int ypos, vector<terrain> tile,vector<object> enemies, basicmovable &player,int &xmove, int &ymove, int index)
{
    ...
    {
        case1 = 0;
    	while(tile[i].type != 0 && collide(xpos+xmove, ypos+ymove,UNIT,UNIT,tile[i].xpos(), tile[i].ypos(),TILE,TILE))
    	{
            if(case1 == 0){
        		if(xmove > 0)
                    xmove--;
    ...
    for(int i =  0 ; i < enemies.size(); i++)
    {
        if(!(enemies[i].isDead()) || enemies[i].death_animation)
        {
            ...
            originx = xmove;
            originy = ymove;
        	while(index != i && collide(xpos+xmove, ypos+ymove,UNIT,UNIT,enemies[i].xpos(), enemies[i].ypos(),UNIT,UNIT))
        	{
                if(case1 == 0){
            		if(xmove > 0)
                        xmove--;
    ...
    if(index != -1)
    {
        originx = xmove;
        originy = ymove;
    	while(collide(xpos+xmove,ypos+ymove,UNIT,UNIT,player.xpos(), player.ypos(),UNIT,UNIT))
    	{
            if(case1 == 0){
        		if(xmove > 0)
                    xmove--;
